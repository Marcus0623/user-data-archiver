...
powershell -NoProfile -Command "if (-not (Test-Path -LiteralPath 'user-data-archiver.exe')) { exit 1 }; $t = (Get-Item -LiteralPath 'user-data-archiver.exe').LastWriteTime; if (Get-ChildItem -File | Where-Object { $_.Extension -in '.go','.manifest','.syso' -and $_.LastWriteTime -gt $t }) { exit 1 }; if (Get-ChildItem -File -Filter '*.exe' | Where-Object { $_.Name -ne 'user-data-archiver.exe' }) { exit 1 }; exit 0"
if errorlevel 1 (
...
start /wait "" "%~dp0user-data-archiver.exe" %*
echo.
pause
